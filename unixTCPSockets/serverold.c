...
int th_count = 0;
int status;
pthread_t thread[NUM];//initialize thread array to 10
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        pthread_attr_init(&thread_attr[th_count]);
        pthread_attr_setscope(&thread_attr[th_count], PTHREAD_SCOPE_SYSTEM);
        if( (status = pthread_create(&thread[th_count], &thread_attr[th_count], word_game, &newsockfd)) < 0 )//pthread create returns an int while running a program thread_routine
        {
            printf("P: VERY BAD NEWS:  pthread_create failed!\n");
            exit(-1);
        }
        status = pthread_join(thread[th_count], &retval);
